...
const Parsers = {
  ...
  // ============ GROWW P&L STATEMENT ============
  // P&L = realized gains history → stored as pnlRecords, NOT added to net worth assets
    ...
    return new Promise((resolve) => {
      Papa.parse(text, {
        ...
        complete: (results) => {
          const pnlRecords = [];
          results.data.forEach(row => {
            // Groww P&L export columns
            const name       = row['Stock Name'] || row['Fund Name'] || row['Instrument'] || row['Name'];
            const sellDate   = row['Sell Date'] || row['Date'] || row['Transaction Date'];
            const buyPrice   = parseFloat(row['Buy Price'] || row['avg_buy_price'] || 0);
            const sellPrice  = parseFloat(row['Sell Price'] || row['sell_price'] || 0);
            const qty        = parseFloat(row['Quantity'] || row['qty'] || 0);
            const gain       = parseFloat(row['P&L'] || row['pnl'] || row['Gain/Loss'] || row['Realised P&L'] || 0);
            const gainType   = detectGainType(row['Type'] || row['Gain Type'] || '', sellDate);
            ...
            if (name && qty > 0) {
              pnlRecords.push({
                ...
                qty,
                gain,
                gainType,  // 'LTCG' | 'STCG' | 'unknown'
              });
            }
          });

          // P&L = zero assets (realized gains don't add to current holdings)
          resolve({
            assets: [],
            pnlRecords,
            source: 'groww_pnl',
            ...
            type: 'groww_pnl',
            raw: results.data,
            message: `Found ${pnlRecords.length} realized P&L records. These are stored for tax reference and do not affect net worth.`
          });
        },
        error: (err) => resolve({ assets: [], pnlRecords: [], error: err.message })
      });
